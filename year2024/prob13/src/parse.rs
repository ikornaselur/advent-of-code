...
fn nom_button(input: &str) -> IResult<&str, Button> {
    let (input, button_id) = preceded(tag("Button "), one_of("AB"))(input)?;
    let (input, x) = preceded(tag(": X+"), nom_unsigned_digit)(input)?;
    let (input, y) = preceded(tag(", Y+"), nom_unsigned_digit)(input)?;
...
fn nom_prize(input: &str) -> IResult<&str, (u64, u64)> {
    let (input, x) = preceded(tag("Prize: X="), nom_unsigned_digit)(input)?;
    let (input, y) = preceded(tag(", Y="), nom_unsigned_digit)(input)?;
...
fn nom_problem(input: &str) -> IResult<&str, Problem> {
    ...
    let (input, b) = preceded(newline, nom_button)(input)?;
    let (input, prize) = preceded(newline, nom_prize)(input)?;

    Ok((input, Problem { a, b, prize }))
}

pub fn parse_input(input: &str) -> Result<Vec<Problem>> {
    let (_, problems) = separated_list1(many1(newline), nom_problem)(input)
